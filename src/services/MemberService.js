...
class MemberService {
  ...
  async createMember(memberData, brandId, userId, context = {}) {
    try {
      ...
      const memberId = memberData.member_id || await this.generateMemberId(brandId);
      ...
      const memberToCreate = {
        ...
        total_points_earned: memberData.points_balance || 0,
        created_by: userId
      };
      ...
      await this.auditLogRepository.logUserAction({
        ...
        metadata: {
          ...
          initialPoints: member.points_balance
        }
      });
  ...
  async getMemberById(memberId, brandId) {
    try {
      ...
      logger.error('Get member failed', {
        error: error.message,
        memberId,
        brandId
      });
